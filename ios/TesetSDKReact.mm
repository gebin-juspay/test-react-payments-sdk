...
#import <TesetPaymentsServices/TesetPaymentsServices.h>

__weak static TesetPaymentsServices *_hyperServicesReference;
...
RCT_EXPORT_METHOD(preFetch:(NSString *)data) {
    if (data && data.length>0) {
        @try {
            NSDictionary *jsonData = [TesetSDKReact stringToDictionary:data];
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                [TesetPaymentsServices preFetch:jsonData];
            } else {
...
RCT_EXPORT_METHOD(createHyperServices) {
    if (self.hyperInstance == NULL) {
        self.hyperInstance = [[TesetPaymentServices alloc] init];
    ...
    if (data && data.length>0) {
        @try {
            ...
            if (jsonData && [jsonData isKindOfClass:[NSDictionary class]] && jsonData.allKeys.count>0) {
                ...
                __weak TesetSDKReact *weakSelf = self;
...
{
    TesetPaymentsServices *hyperServicesInstance = _hyperServicesReference;
